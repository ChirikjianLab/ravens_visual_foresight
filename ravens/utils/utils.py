...
def perturb_pp(input_images, pixels, set_theta_zero=False):
  """Data augmentation on images."""
  image_size = input_images[0].shape[:2]
  ...
  for input_image in input_images:
    new_input_images.append(cv2.warpAffine(
        input_image,
        transform[:2, :], (image_size[1], image_size[0]),
        flags=cv2.INTER_NEAREST))
  return new_input_images, new_pixels, new_rounded_pixels, transform_params

def perturb_pp_seq(init_images, target_images, p0_list, p1_list, set_theta_zero=False):
  """Data augmentation on images."""
  image_size = init_images[0].shape[:2]
  ...
  while True:
    ...
    for pixel in p0_list:
      ...
      new_pixels.append(pixel)
      new_p0_list.append(rounded_pixel)

    for pixel in p1_list:
      pixel = np.float32([pixel[1], pixel[0], 1.]).reshape(3, 1)
      ...
      new_pixels.append(pixel)
      new_p1_list.append(rounded_pixel)
  ...
  for init_image in init_images:
    new_init_images.append(cv2.warpAffine(
        init_image,
        transform[:2, :], (image_size[1], image_size[0]),
        flags=cv2.INTER_NEAREST))
  new_target_images = []
  for target_image in target_images:
    new_target_images.append(cv2.warpAffine(
        target_image,
        transform[:2, :], (image_size[1], image_size[0]),
        flags=cv2.INTER_NEAREST))
  return new_init_images, new_target_images, new_p0_list, new_p1_list, transform_params
