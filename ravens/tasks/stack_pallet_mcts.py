...
class StackPalletMCTS(TaskMCTS):
  """Stack a pallet (mcts)."""
  ...
  def reset(self, env):
    ...
    base_urdf = 'stand/stand_square.urdf'
    ...
    place_pos = [
        ...
        (0.05,    0, self.block_size[-1] / 2),
        (0.05, 0.05, self.block_size[-1] / 2),
        (   0,    0, self.block_size[-1] / 2 + self.block_size[-1]),
        (   0, 0.05, self.block_size[-1] / 2 + self.block_size[-1]),
        (0.05,    0, self.block_size[-1] / 2 + self.block_size[-1]),
        (0.05, 0.05, self.block_size[-1] / 2 + self.block_size[-1])]
    primitives = [0, 0, 0, 0, 1, 1, 1, 1]
    ...
    # Goal
    if self.mode == 'train':
      self.goals.append((objs[:4], np.ones((4, 4)), targs[:4],
          False, True, 'pose', None, 1 / 2, primitives[:4]))
      self.goals.append((objs[4:], np.ones((4, 4)), targs[4:],
          False, True, 'pose', None, 1 / 2, primitives[4:]))
    else:
