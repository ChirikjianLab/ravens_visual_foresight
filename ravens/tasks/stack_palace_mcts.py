...
class StackPalaceMCTS(TaskMCTS):
  """Stacking palace task (mcts)."""
  ...
  def reset(self, env):
    ...
    if seq_idx == 0:
      ...
      place_pos = [(0,    0,  block_size[-1] / 2 + 0 * block_size[-1]), 
                   ...
                   (0, 0.025, block_size[-1] / 2 + 2 * block_size[-1]),
                   (0, 0.075, block_size[-1] / 2 + 2 * block_size[-1])]
      primitives = [0, 0, 0, 2, 2, 1, 1]
      ...
      if self.mode == 'train':
        ...
        self.goals.append((objs[:3], np.ones((3, 3)), targs[:3],
            False, True, 'pose', None, 3 / 7, primitives[:3]))
        self.goals.append((objs[3:5], np.ones((2, 2)), targs[3:5],
            False, True, 'pose', None, 2 / 7, primitives[3:5]))
        self.goals.append((objs[5:], np.ones((2, 2)), targs[5:],
            False, True, 'pose', None, 2 / 7, primitives[5:]))
      else:
      ...
      if self.mode == 'train':
        ...
        self.goals.append((objs[:2], np.ones((2, 2)), targs[:2],
            False, True, 'pose', None, 2 / 7, primitives[:2]))
        self.goals.append((objs[2:3], np.ones((1, 1)), targs[2:3],
            False, True, 'pose', None, 1 / 7, primitives[2:3]))
        self.goals.append((objs[3:4], np.ones((1, 1)), targs[3:4],
            False, True, 'pose', None, 1 / 7, primitives[3:4]))
        self.goals.append((objs[4:5], np.ones((1, 1)), targs[4:5],
            False, True, 'pose', None, 1 / 7, primitives[4:5]))
        self.goals.append((objs[5:], np.ones((2, 2)), targs[5:],
            False, True, 'pose', None, 2 / 7, primitives[5:]))     
      else:
        self.goals.append((objs, np.ones((num_blocks, num_blocks)), targs,
