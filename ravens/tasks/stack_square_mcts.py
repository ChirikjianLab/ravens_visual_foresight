...
class StackSquareMCTS(TaskMCTS):
  """Stack square task (mcts)."""
    ...
    if seq_idx == 0:
      ...
      if self.mode == 'train':
        ...
        self.goals.append((objs[:2], np.ones((2, 2)), targs[:2],
            False, True, 'pose', None, 1 / 2, primitives[:2]))
        self.goals.append((objs[2:], np.ones((2, 2)), targs[2:],
            False, True, 'pose', None, 1 / 2, primitives[2:]))
      else:
      ...
      if self.mode == 'train':
        ...
        self.goals.append((objs[:1], np.ones((1, 1)), targs[:1],
            False, True, 'pose', None, 1 / 4, primitives[:1]))
        self.goals.append((objs[1:2], np.ones((1, 1)), targs[1:2],
            False, True, 'pose', None, 1 / 4, primitives[1:2]))
        self.goals.append((objs[2:3], np.ones((1, 1)), targs[2:3],
            False, True, 'pose', None, 1 / 4, primitives[2:3]))
        self.goals.append((objs[3:], np.ones((1, 1)), targs[3:],
            False, True, 'pose', None, 1 / 4, primitives[3:]))
      else:
        self.goals.append((objs, np.ones((num_blocks, num_blocks)), targs,
            False, True, 'pose', None, 1, primitives))
