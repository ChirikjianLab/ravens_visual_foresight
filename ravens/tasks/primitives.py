...
class PickPlace():
  ...
  def __call__(self, movej, movep, ee, pose0, pose1):
    ...
    while not ee.detect_contact():  # and target_pose[2] > 0:
      ...
      if timeout or (time.time() - t0) > self.pick_place_timeout:
        return True
...
class PickPlaceDiscrete():
  """Pick and place primitive for dicrete call."""

  def __init__(self, height=0.32, speed=0.01):
    self.height, self.speed = height, speed
    self.pick_place_timeout = 2

  def __call__(self, movej, movep, ee, pose, type):
    """Execute pick and place primitive.
    ...
    Args:
      ...
      movep: function to move robot end effector pose.
      ee: robot end effector.
      pose: SE(3) picking / placing pose.
      type: type of the action, PICk / PLACE
    ...
    if type == PICK:
      ...
      preplace_to_place = ((0, 0, self.height), (0, 0, 0, 1))
      postplace_to_place = ((0, 0, 0.32), (0, 0, 0, 1))
      preplace_pose = utils.multiply(pose, preplace_to_place)
...
# class PickPlaceDynamics(PickPlace):
#   """An R^2 pick-and-place primitive with no rotation DoF (Dynamics)."""
...
#     pick_pose[1] = (0, 0, 0, 1)
#     place_pose = pose1
#     place_pose[1] = (0, 0, 0, 1)
#     timeout = super().__call__(movej, movep, ee, pick_pose, place_pose)
#     return timeout